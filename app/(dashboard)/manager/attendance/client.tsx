...
export function ManagerAttendanceClient({ date, rows, employees: _ }: Props) {
  ...
  const summary = useMemo(() => ({
    total:     rows.length,
    checked:   rows.filter(r => r.status !== 'not_started').length,
    out:       rows.filter(r => r.status === 'checked_out').length,
    notStart:  rows.filter(r => r.status === 'not_started').length,
  }), [rows])

  function onDateChange(d: string) {
    router.push(`/manager/attendance?date=${d}`)
  }
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: '전체 직원',  value: summary.total,    color: 'text-slate-700' },
          { label: '미출근',     value: summary.notStart, color: 'text-slate-400' },
          { label: '출근완료',   value: summary.checked,  color: 'text-emerald-600' },
          { label: '퇴근완료',   value: summary.out,      color: 'text-blue-600' },
        ].map(s => (
