'use client'
/* ================================================================
   InvitePasswordForm — 비밀번호 설정 클라이언트 컴포넌트
   /auth/invite 페이지에서 토큰이 검증된 후 렌더링
================================================================ */
...
export function InvitePasswordForm({ token, email, name }: Props) {
    ...
    return (
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-xl">
          ...
          <p className="text-sm text-slate-500 mb-2">{name}님, 환영합니다.</p>
          <p className="text-sm text-slate-400 mb-6">
            등록하신 이메일과 비밀번호로 로그인하세요.
          </p>
          <Link
            href="/login"
            className="inline-flex items-center justify-center gap-2 w-full bg-blue-600 text-white text-sm font-semibold px-4 py-3 rounded-xl hover:bg-blue-700 transition-colors"
          >
            로그인 하러 가기 →
          </Link>
        </div>
    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          {/* 비밀번호 */}
          <Field label="비밀번호 설정" error={fieldErrors.password} hint="8자 이상" required>
          ...
          {/* 비밀번호 확인 */}
          <Field label="비밀번호 확인" error={fieldErrors.passwordConfirm} required>
            <input
              type={showPw ? 'text' : 'password'}
          ...
          >
            {step === 'loading' ? (
              <><Loader2 size={16} className="animate-spin" />가입 처리 중...</>
            ) : (
              '가입 완료'
            )}
          </button>
    ...
    <div className="flex gap-3">
      {checks.map(c => (
        <div key={c.label} className="flex items-center gap-1">
          <span className={c.ok ? 'text-emerald-500' : 'text-slate-300'}>
            <CheckCircle2 size={11} />
          </span>
          <span className={`text-[11px] ${c.ok ? 'text-emerald-600' : 'text-slate-400'}`}>
            {c.label}
          </span>
        </div>
      ))}
