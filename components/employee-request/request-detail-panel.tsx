'use client'
/* ================================================================
   RequestDetailPanel — 가입신청 상세 + 저장 결과 확인 영역
================================================================ */
      ...
      <div className="flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {r.status === 'approved' && (
            <div className="flex items-start gap-2.5 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
              ...
              <p className="text-xs text-emerald-700">
                승인 완료. 초대 이메일 발송 후 직원이 비밀번호를 설정하면 계정이 활성화됩니다.
              </p>
            </div>
          )}
          ...
          <InfoSection icon={<CalendarDays size={14} className="text-blue-500" />} title="신청 정보">
            <InfoGrid rows={[
              ['회사명', r.companyName],
              ['신청자', r.requestedByName],
              ['신청일', formatDateDot(r.createdAt.slice(0, 10))],
            ]} />
          </InfoSection>

          {/* ★ DB 저장 결과 확인 영역 (승인/거절 후 자동 표시) */}
            ...
            <div className="rounded-xl border border-dashed border-slate-300 overflow-hidden">
              ...
              {showDbInfo && (
                <div className="px-4 py-3 bg-slate-900 text-[11px] font-mono space-y-1.5">
                  <DbRow label="id"            value={r.id} />
                  <DbRow label="status"        value={r.status}          highlight={r.status === 'approved' ? 'green' : 'red'} />
                  <DbRow label="reviewed_by"   value={r.reviewedBy ?? 'null'} />
                  <DbRow label="reviewed_at"   value={r.reviewedAt
                    ? new Date(r.reviewedAt).toLocaleString('ko-KR')
                    : 'null'}
                  />
                  <DbRow label="reject_reason" value={r.rejectionReason ?? 'null'} />
                </div>
              )}
            </div>
          )}
            ...
            {confirmOpen ? (
              <div className="space-y-2.5">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-xs text-emerald-800">
                  <span className="font-semibold">{r.name}</span>의 가입신청을 승인하시겠습니까?<br />
                  승인 즉시 초대 이메일이 발송됩니다.
                </div>
