'use client'
/* ================================================================
   RequestPage — 어드민 가입신청 승인/거절 오케스트레이터
   Server Actions → 낙관적 업데이트 → 서버 재동기화
================================================================ */
...
/* ── Toast ─────────────────────────────────────────────── */
interface Toast { id: string; type: 'success' | 'error'; title: string; body: string }
...
export function RequestPage({ initialRequests, currentUserId }: Props) {
  ...
  async function handleApprove(id: string) {
    ...
    const result = await approveEmployeeRequest(Number(id))

    if (result.success) {
      toast('success', '승인 완료', '가입신청이 승인되었습니다. 초대 이메일 발송 예정입니다.')
      refreshRequests()
    } else {
      // 롤백
  ...
  async function handleReject(id: string, reason: string) {
    ...
    if (result.success) {
      toast('success', '거절 처리 완료', '가입신청이 거절되었습니다.')
      refreshRequests()
    } else {
  ...
  function handleSelect(r: EmployeeRequest) {
    setSelected(r)
    setShowDetail(true)
  }
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2 w-80 max-w-[calc(100vw-2rem)] pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={cn(
            ...
            t.type === 'success' ? 'bg-emerald-600 border-emerald-500' : 'bg-red-600 border-red-500',
          )}>
            {t.type === 'success'
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/80 mt-0.5">{t.body}</p>
            </div>
            <button onClick={() => remove(t.id)} className="text-white/70 hover:text-white mt-0.5">
              <X size={14} />
            </button>
          </div>
        ))}
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            기업담당자가 등록한 직원 가입신청을 검토하고 승인 또는 거절합니다
          </p>
        </div>
        {isPending && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-5 items-start">
        ...
        <div className={cn('lg:sticky lg:top-6', !showDetail && 'hidden lg:block')}>
          {selected ? (
            ...
            <div className="hidden lg:flex flex-col items-center justify-center h-72 bg-white rounded-2xl border border-dashed border-slate-200 text-slate-400">
