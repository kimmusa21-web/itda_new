...
import { updateCompanyPayslipNote, updateCompanyPayslipNoteById } from '@/lib/actions/company-actions'
import { DEFAULT_PAYSLIP_NOTE_PLACEHOLDER } from '@/lib/payslip-defaults'

interface Props {
  initialNote: string | null
  /** 어드민: companyId를 넘기면 updateCompanyPayslipNoteById 사용. 없으면 매니저용 updateCompanyPayslipNote 사용 */
  companyId?: number
}

export function PayslipNoteEditor({ initialNote, companyId }: Props) {
  ...
  async function handleSave() {
    ...
    const result = companyId != null
      ? await updateCompanyPayslipNoteById(companyId, note || null)
      : await updateCompanyPayslipNote(note || null)
    if (!result.success) {
