    ...
    <div className="card overflow-hidden">
      <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {errs.length  > 0 && <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700">{errs.length}건 오류</span>}
          {warns.length > 0 && <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">{warns.length}건 경고</span>}
        </div>
      </div>
          ...
          <div key={i} className={cn('flex items-start gap-3 px-5 py-3',
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {err.email && <span className="text-xs font-mono text-slate-400 truncate">{err.email}</span>}
              </div>
              <p className={cn('text-xs mt-0.5',
                err.severity === 'error' ? 'text-red-600' : 'text-amber-700')}>{err.reason}</p>
            </div>
          </div>
        ))}
      </div>
