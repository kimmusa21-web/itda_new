...
export function PayrollPreviewList({ previews }: Props) {
  ...
  const visible = showAll ? previews : previews.filter(p => p.status === 'valid').slice(0, 5)
      ...
      <div className="divide-y divide-slate-50">
        {visible.map(p => {
          ...
          const badgeCls =
            ...
            p.status === 'ignored' ? 'bg-amber-100 text-amber-700' :
            'bg-red-100 text-red-700'
          const badgeLabel = p.status === 'valid' ? '정상' : p.status === 'ignored' ? '무시' : '오류'
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full', badgeCls)}>{badgeLabel}</span>
                  </div>
                ...
                <div className="px-5 pb-4 border-t border-slate-100 bg-white">
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    ...
                    <div>
                      ...
                      <div className="flex justify-between text-xs font-bold border-t pt-1 mt-1">
                        <span className="text-rose-600">합계</span>
