...
import {
  Upload, ShieldCheck, CheckCircle2, RefreshCw, Loader2,
...
import type { ColumnMapping, CsvRow, ValidationResult, PreviewRow } from '@/types/payroll-upload'
import type { EmployeeMaster } from '@/types/payroll-upload'
...
export function PayrollUploadPage({ companies, currentUserId }: Props) {
  /* 설정 */
  ...
  const [dropStatus,    setDropStatus]    = useState<DropzoneStatus>('idle')
  ...
  const handleCompanyChange = useCallback(async (id: number) => {
    setCompanyId(id)
    if (!id) { setMappings([]); setEmployees([]); return }
    setLoadingMeta(true)
  ...
  function handleFile(file: File) {
    ...
    Papa.parse<CsvRow>(file, {
      ...
      error: () => {
        setDropStatus('idle')
        alert('CSV 파일을 읽을 수 없습니다. UTF-8 인코딩인지 확인해주세요.')
      },
    })
  }
  ...
  function handleValidate() {
    ...
    const prevs  = transformCsvRows(csvRows, mappings, employees, companyId, accrualMonth, paymentDate || null)
  ...
  function handleReset() {
    setFileName(''); setDropStatus('idle'); setCsvRows([]); setCsvHeaders([])
    setValidation(null); setPreviews([]); setPhase('setup'); setResultMsg('')
  }

  const canValidate = companyId > 0 && !!accrualMonth && csvRows.length > 0 && mappings.length > 0
  const canUpload   = phase === 'validated' && (validation?.canUpload ?? false)
    ...
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          검증 후 pay_info에 upsert 저장됩니다. 같은 월 재업로드 시 자동 덮어쓰기.
        </p>
      </div>

      {/* 완료 / 실패 배너 */}
      {(phase === 'done' || phase === 'failed') && (
        <div className={cn('flex items-start gap-3 px-5 py-4 rounded-2xl',
          phase === 'done'
          ...
          <div className="flex-1">
            ...
            <p className="text-xs mt-0.5 opacity-80">{resultMsg}</p>
          </div>
          <button onClick={handleReset} className="text-white/70 hover:text-white text-sm">
            ...
            <div key={s} className="flex items-center flex-1">
              <div className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className={cn('text-[10px] font-medium',
                  stepIdx >= i ? 'text-blue-600' : 'text-slate-400')}>{s}</span>
              </div>
              {i < 2 && <div className={cn('h-0.5 flex-1 mx-1 mb-4',
