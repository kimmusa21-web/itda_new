...
import { PAYSLIP_HEADER_LABELS, REQUIRED_PAYSLIP_HEADERS } from '@/types/payslip-csv-upload'
...
export function PayslipCsvUpload({ role, defaultCompanyId, companies = [] }: Props) {
  ...
  async function handleUpload() {
    ...
    setStep('uploading')
    try {
      const res = await uploadPayslipCsv({
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5 bg-blue-50 border-blue-100 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-blue-800">급여 CSV 업로드 안내</p>
        </div>
        <ul className="text-xs text-blue-700 space-y-1 ml-5 list-disc">
          <li>
            필수 컬럼: {REQUIRED_PAYSLIP_HEADERS.map(h => PAYSLIP_HEADER_LABELS[h]).join(', ')}
          </li>
          <li>귀속월 형식: <code className="bg-blue-100 px-1 rounded">YYYY-MM</code> (예: 2026-04)</li>
          <li>지급일 형식: <code className="bg-blue-100 px-1 rounded">YYYY-MM-DD</code> (선택)</li>
              ...
              <table className="w-full text-xs" style={{ minWidth: '680px' }}>
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-3 py-2.5 text-right text-slate-500 font-semibold">상여금</th>
                    <th className="px-3 py-2.5 text-right text-slate-500 font-semibold">공제액</th>
                  ...
                  {rows.map((row, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-slate-700">{row.email || <span className="text-red-400 italic">없음</span>}</td>
                        <td className="px-3 py-2 text-slate-600">{row.pay_month || <span className="text-red-400 italic">없음</span>}</td>
                        <td className="px-3 py-2 text-right text-slate-600">
                          {row.base_salary ? Number(row.base_salary).toLocaleString() : <span className="text-red-400 italic">없음</span>}
                        </td>
                        <td className="px-3 py-2 text-right text-slate-500">
                          {row.bonus ? Number(row.bonus).toLocaleString() : '0'}
                        </td>
                        <td className="px-3 py-2 text-right text-slate-500">
                          {row.deduction ? Number(row.deduction).toLocaleString() : '0'}
                        </td>
            ...
            <div className="flex items-start gap-2.5 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
              ...
              <div>
                <p className="text-sm font-semibold text-red-700">
                  오류 {invalidRowCount}건 — 업로드 불가
                </p>
                <p className="text-xs text-red-600 mt-0.5">
                  CSV 파일의 오류를 수정 후 다시 업로드해주세요.
                  한 행이라도 오류가 있으면 전체 업로드가 차단됩니다.
                </p>
              </div>
            </div>
          )}
...
function UploadResultPanel({
  ...
  const isFailed  = result.successCount === 0
